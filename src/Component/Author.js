  ...
  return (
    <div className="container">
      {authorsState.authors.loading ? (
        ...
        authorsState.authors.slice(pagination.start,pagination.end).map((author) => {
          return (
            <div key={author.id}>
                ...
                <Card className="border border-primary m-2 text-white cardcolor">
                  <CardBody>
                    <CardTitle className="font-weight-bold">
                      <i className="fa fa-user-circle-o"></i> {author.firstName} {author.lastName}
                    </CardTitle>
                  </CardBody>
                </Card>
              </Link>
      ...
      <AuthorListPagination showPerPage={showPerPage} onPaginationChange={onPaginationChange} totalAuthors={authorsState.authors.length} />
    </div>
  );
};
